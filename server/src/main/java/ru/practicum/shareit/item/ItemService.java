...
public class ItemService {
    ...
    public List<ItemDto> getAllUsersItem(Integer from, Optional<Integer> size, Long userId) {
        ...
        return itemStorage.getAllUserItems(userId, Paging.paging(from, size)).stream().sorted(Comparator.comparing(Item::getId)).peek(this::updateLastNextBooking)
    ...
    public CommentDto createComment(Long userId, Long itemId, Comment comment) {
        ...
        try {
            if (bookingService.getBookingByUser(0, Optional.empty(), userId, State.PAST).stream()
