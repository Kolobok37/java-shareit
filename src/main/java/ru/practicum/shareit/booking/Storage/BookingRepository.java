...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    @Query(value = "select * " +
            ...
            "left join users as us on b.booker_id = us.user_id " +
            "left join items as i on b.item_id = i.item_id " +
            "where b.booker_id=:userId AND (b.status='APPROVED' OR b.status='WAITING') AND b.start_date > NOW() ",
            nativeQuery = true)
    ...
    @Query(value = "select * " +
            ...
            "left join users as us on b.booker_id = us.user_id " +
            "left join items as i on b.item_id = i.item_id " +
            "where b.booker_id=:userId AND b.status='APPROVED' AND b.end_date < NOW() ",
            nativeQuery = true)
    ...
    @Query(value = "select * " +
            ...
            "left join users as us on b.booker_id = us.user_id " +
            "left join items as i on b.item_id = i.item_id " +
            "where i.owner_id=:userId AND (b.status='APPROVED' OR b.status='WAITING') AND b.start_date > NOW() ",
            nativeQuery = true)
    ...
    @Query(value = "select * " +
            ...
            "left join users as us on b.booker_id = us.user_id " +
            "left join items as i on b.item_id = i.item_id " +
            "where i.owner_id=:userId AND b.status='APPROVED' AND b.end_date < NOW() ",
            nativeQuery = true)
    ...
    @Query(value = "select * " +
            ...
            "left join users as us on b.booker_id = us.user_id " +
            "left join items as i on b.item_id = i.item_id " +
            "where i.owner_id=:userId  AND NOW() between b.start_date AND b.end_date ", //AND b.status='APPROVED'
            nativeQuery = true)
